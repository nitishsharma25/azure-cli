...
from ._util import resolve_poller, generate_missing_parameters, get_mysql_list_skus_info, generate_password, parse_maintenance_window, \
    ...
    run_subprocess_get_output, fill_action_template, get_git_root_dir, get_single_to_flex_sku_mapping, parse_firewall_rules, GITHUB_ACTION_PATH
...
def flexible_server_import_create(cmd, client,
    ...
    if data_source_type.lower() == 'mysql_single' and create_mode.lower() == 'migrate':
        copy_firewall_rules_from_single_to_flex(db_context=db_context, cmd=cmd, source_server_id=source_server_id, target_server_name=server_name)
...
def copy_firewall_rules_from_single_to_flex(db_context, cmd, source_server_id, target_server_name):
    id_parts = parse_resource_id(source_server_id)
    single_firewall_client = cf_mysql_firewall_rules(cli_ctx=cmd.cli_ctx, _=None)
    single_firewall_rules = single_firewall_client.list_by_server(id_parts['resource_group'], id_parts['name'])
    result = parse_firewall_rules(single_firewall_rules)
    for rule in result:
