...
from azure.cli.core.azclierror import ValidationError, ArgumentUsageError, ClientRequestError, InvalidArgumentValueError
...
def mysql_import_create_data_source_validator(cli_ctx, source_server_id, resource_group_name):
    ...
    if id_parts['resource_group'] != resource_group_name:
        raise ClientRequestError('Different resource groups were found for single and flexible server. '
    ...
    if id_parts['subscription'] != get_subscription_id(cli_ctx):
        raise ClientRequestError('Migrations are only supported under the same subscription. ')
